import type { ParsedUrlInfo } from "@/types/url";
...
export function base64Decode(decode: string): string {
  ...
  if (typeof window === "undefined") {
    return Buffer.from(decode, "base64").toString("utf-8");
  }
...
export function base64Encode(encode: string): string {
  if (typeof window === "undefined") {
    return Buffer.from(encode, "utf-8").toString("base64");
  }
...
export const urlToParsedQueryParamValues = (url: URL) =>
  url.search
    .split("?")[1]!
    .split(/([&])/)
    .map((x, i) => (i % 2 === 0 || 0 ? x : null))
    .filter(x => x != null)
    .map(val => ({
      [`${val?.split(/([=])/)[2]}`]: `${val?.split(/([=])/)[2]}`
    }));
